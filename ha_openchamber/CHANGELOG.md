...
- **Installs an interactive toolset for the terminal** — the image only carried what the server needed, so the built-in terminal lacked basics (`ps`, `vim`, `jq`, archives, …). Adds the requested toolset: `awscli`, `bats` (+ assert/file/support), `bc`, `bubblewrap`, `coreutils`, `direnv`, `file`, `findutils`, `fzf`, `gawk`, `gettext`, `grc`, `highlight`, `ipcalc`, `jo`, `jq`, `moreutils`, `ncdu`, `netcat-openbsd`, `openssl`, `p7zip`, `pigz`, `procps`, `progress`, `psutils`, `python3` (+ virtualenv/virtualenvwrapper), `sqlite3`, `sudo`, `telnet`, `tree`, `unzip`, `vim`, `wget`, `zoxide`, plus `-dev` headers (`libffi`, `libpcre2`, `libpq`, `libyaml`) and `libtool`. Image grows accordingly (~300 MB installed); `git` arrived in 1.1.2.
