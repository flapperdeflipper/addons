...
- **Changed — the add-on is simplified for its actual owner**: everything that used to be an option that was never changed is hardcoded on — full MCP tool profile, native MCP bridge (assist), LSP, screenshot tool, focus-friendly responses, install briefing, sensitive-file read restriction, add-on folder guidance, CPU auto-detection, terminal appearance (Breeze, 14px, block cursor, no blink). The LAN server (4096, used by OpenChamber) and the 8927 MCP endpoint now run unconditionally. Remaining options: `access_token`, `mcp_hub_url`, `mcp_http_token`, `server_password`, `cors_origins`, `z2m_url`, `z2m_mqtt_topic`, `env_vars`, `opencode_config`.
- **Removed — PPQ private mode**: the s6 proxy service, both options and the provider config block. It was disabled with no API key on this install; git history has it if it ever comes back.
- **Removed — startup hooks option**: hooks are always available; a `startup.d` folder that is empty or absent runs nothing.
- **Removed — legacy migration shims and unused knobs**: the pre-2.x update-policy and interface-mode notices, the AGENTS-hash legacy marker migration, `serial_devices` (and the `OPENCODE_SERIAL_DEVICES` export), `cpu_mode`, `server_username` (the username is the OpenCode default `opencode`), presentation options, and the `decision_notes_enabled` no-op. Restoring a backup that still carries removed option keys requires re-saving the Configuration tab once after updating.
- **Fixed — read-only session stays read-only in hub mode**: `ha-readonly` now replaces the whole `mcp` section with a session-local compact-profile server and the native bridge off, instead of merging onto the generated config — necessary since the shared hub server is full-profile and cannot be switched per session.
- **Fixed — the 2.22.0 boot crash** (released as 2.22.1 in the branch history): the init script still tested the `HELP_STAMP_FRESH` variable after the help-stamp block that set it was removed, and `set -u` aborted startup; the AGENTS.md ownership-marker write after a refresh was restored in the same pass.
...
- **Fix: the assembled custom config was never loaded** — `config.json` (the `opencode_config` option merged with the configuration-folder drop-ins) was written correctly but nothing ever set `OPENCODE_CONFIG`, and opencode does not read a `config.json` from the config directory on its own. The server and every session therefore ran on the generated MCP/LSP config alone: custom providers were missing from the model picker and OpenChamber could not resolve a runtime credential for them, so small-model actions failed with the provider seemingly configured. The init service now exports `OPENCODE_CONFIG=/data/.config/opencode/config.json` into `/data/.env_vars`, which every OpenCode consumer (LAN server, terminal sessions, MCP services) already sources. A reboot warning disguised as a feature: after updating, restart once and check the model picker.

## 2.17.0

- **Config drop-ins move to the add-on configuration folder** — the drop-in directory from 2.16.0 (`/homeassistant/opencode.d/`) moves to the standardized Home Assistant location for user-provided add-on files: the add-on's own configuration folder, via a new `addon_config` directory mapping. Mounted at `/config` inside the add-on, exposed on the host as `addon_configs/<repository-slug>_ha_opencode` (visible in the Samba `addon_configs` share, included in HA backups). Merge semantics are unchanged: every `*.json` in the folder must be a JSON object, files deep-merge in alphabetical filename order over the `opencode_config` option (a drop-in wins over the option, a later file over an earlier one), an unparseable file is skipped with a named error while the rest still apply, and the assembled `config.json` is written atomically. Move any existing `opencode.d/*.json` files into the configuration folder when updating.

## 2.16.1

- **MCP server quality follow-up** — ESPHome secret redaction now ignores non-string and very short scalar values (a `secrets.yaml` value like `2` or `true` no longer redacts those characters across all ESPHome output); `home-facts` reports packages as "in use" only when `homeassistant.packages:` is actually configured (a stray `packages/` directory no longer produces that briefing line) and the `automations.yaml` briefing bullet says "appears UI-managed" instead of asserting it; `detect_anomalies` sorts error > warning > info instead of warnings first; the native-MCP proxy validates `HA_NATIVE_MCP_TIMEOUT_MS` (0/NaN no longer arms an instant timeout for every request) and survives a client EPIPE; the HTML entity decoder handles `&amp;` last so `&amp;lt;` no longer double-decodes; dead code removed (`getESPHomeDevices`, unused `createResourceLink` import).
- **Fix trace/error-log tests shipped broken in 2.16.0** — `get_error_log` no longer emits `unique`/`duplicates_collapsed` meta keys when dedup was not requested, and `get_automation_trace` timelines are ordered chronologically (trigger before actions) instead of path-alphabetically.
