...
# Fork of alexbelgium/hassio-addons obsidian_syncserver_solo (MIT).
#
...
# Retry budget matches provision.ts: CouchDB can take a while to open its
# listener on first boot.
READY_RETRIES=12
READY_DELAY=5

log() { echo "[obsidian-sync] $*"; }
warn() { echo "[obsidian-sync] WARN: $*" >&2; }
die() {
    echo "[obsidian-sync] ERROR: $*" >&2
    exit 1
}
...
# The password is deliberately NOT printed to the log — Supervisor logs end
# up in diagnostics and support bundles. Read it from the file instead.
# ---------------------------------------------------------------------------
if [[ -z "$PASSWORD" ]]; then
    if [[ -f "$PASSWORD_FILE" ]]; then
        ...
        warn "Retrieve the password on the host with:"
        warn "  cat /addon_configs/*_obsidian-sync/obsidian-syncserver/admin_password"
        warn "Or set a password in the add-on options to choose your own."
    fi
fi
