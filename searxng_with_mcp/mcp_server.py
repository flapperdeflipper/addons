...
class RateLimiter:
    ...
    def can_request(self, url: str) -> bool:
        ...
        self.domain_requests[domain] = [
            ...
            for timestamp in self.domain_requests[domain]
...
class HelperFunctions:
    ...
    def format_text_with_trafilatura(html_content: str, timeout: int) -> str:
        ...
        if not extracted_text:
            ...
            logger.warning(
                "Trafilatura failed/timed out, falling back to basic text extraction."
            )
        lines = [
    ...
    async def process_web_search_result(
        result: Dict[str, Any], valves, client
    ) -> Dict[str, Any]:
        ...
        if valves.IGNORED_WEBSITES:
            ignored_sites = [
                site.strip() for site in valves.IGNORED_WEBSITES.split(",")
            ]
        ...
        try:
            ...
            if kind is not None and kind.mime == "application/pdf":
                ...
                content = HelperFunctions.format_text_with_trafilatura(
                    html_content, valves.TRAFILATURA_TIMEOUT
                )
                truncated_content = HelperFunctions.truncate_to_n_words(
                    content, valves.PAGE_CONTENT_WORDS_LIMIT
                )
...
class MCPEventEmitter:
    ...
    async def emit(
        ...
        await self.send_notification_func(
            "tool/event", {"type": "status", "data": message_data}
        )

    async def emit_citation(
        self, soup: Optional[BeautifulSoup], result_data: Dict[str, Any]
    ):
        pub_date_str = None
        author_str = None
        ...
        await self.send_notification_func(
            "tool/event", {"type": "citation", "data": citation_data}
        )
...
class Tools:
    class Valves(BaseModel):
        ...
        def validate_searxng_url(cls, v):
            if not v.startswith(("http://", "https://")):
                raise ConfigurationError(
                    f"SEARXNG_ENGINE_API_BASE_URL must start with http:// or https://, got {v}"
                )
        ...
        def validate_timezone(cls, v):
            try:
                ...
                raise ConfigurationError(
                    f"Invalid timezone: {v}. Error: {str(e)}. See https://en.wikipedia.org/wiki/List_of_tz_database_time_zones for a list of valid timezones."
                )
    ...
    def __init__(
        self,
        send_notification_func: Optional[Callable[[str, Any], Awaitable[None]]] = None,
    ):
        """
        ...
        for k in script_defaults.keys():
            ...
            if env_value is not None:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid environment variable value for {k.upper()}: {env_value}"
                    )
        ...
        if not file_exists or env_vars_present:
            logger.info(
                f"Saving configuration to {config_path} (reason: {'first run' if not file_exists else 'env vars provided'})"
            )
        ...
        logger.info(
            f"Website cache initialized: maxsize={self.valves.CACHE_MAXSIZE}, ttl={self.valves.CACHE_TTL_MINUTES} minutes"
        )
        self.headers = {
    ...
    async def search_web(
        ...
        try:
            await self.emitter.emit(
                f"🌐 Connecting to SearXNG at {self.valves.SEARXNG_ENGINE_API_BASE_URL}",
                step_number=4,
            )
            resp = await self.client.get(
                self.valves.SEARXNG_ENGINE_API_BASE_URL, params=params, timeout=120
            )
            resp.raise_for_status()
            data = resp.json()
            results = data.get("results", [])
            await self.emitter.emit(
                f"📊 {len(results)} raw results found for category '{safe_category}'",
                step_number=5,
            )
        except httpx.HTTPStatusError as e:
            error_msg = f"SearXNG search failed: {e.response.status_code} {e.response.reason_phrase}"
            await self.emitter.emit(
                status="error", description=f"❌ {error_msg}", done=True
            )
        ...
        if not results:
            await self.emitter.emit(
                status="complete",
                description="✅ Search complete - No results found.",
                done=True,
                step_number=6,
            )
        ...
        if safe_category == "images":
            ...
            for i, result in enumerate(results[:category_limit]):
                ...
                if self.valves.CITATION_LINKS:
                    ...
                    await self.emitter.emit_citation(
                        None, {"content": citation_content, **img_data}
                    )
            final_results_count = len(processed_results)
            await self.emitter.emit(
                status="complete",
                description=f"✅ Image search complete - {final_results_count} images returned.",
                done=True,
                step_number=7,
            )
            ...
            await self.emitter.emit(
                status="complete",
                description=f"✅ Video search complete - {final_results_count} videos returned.",
                done=True,
                step_number=7,
            )
            ...
            await self.emitter.emit(
                status="complete",
                description=f"✅ File search complete - {final_results_count} files returned.",
                done=True,
                step_number=7,
            )
            ...
            for i, result in enumerate(results[:limit]):
                ...
                if map_data.get("latitude") and map_data.get("longitude"):
                    output_parts.append(
                        f"**Coordinates:** {map_data['latitude']}, {map_data['longitude']}"
                    )
            ...
            await self.emitter.emit(
                status="complete",
                description=f"✅ Map search complete - {final_results_count} locations returned.",
                done=True,
                step_number=7,
            )
            ...
            await self.emitter.emit(
                status="complete",
                description=f"✅ Social media search complete - {final_results_count} posts/profiles returned.",
                done=True,
                step_number=7,
            )
        else:
            await self.emitter.emit(
                "🔄 Processing web results by scraping pages", step_number=6
            )
            ...
            for result in limited_results_for_scraping:
                ...
                if url:
                    ...
                    if not rate_limiter.can_request(url):
                        ...
                        logger.warning(
                            f"Rate limit exceeded for {domain}. Skipping result. Try again in {wait_time:.1f} seconds."
                        )

                        continue

                tasks.append(
                    HelperFunctions.process_web_search_result(
                        result, self.valves, self.client
                    )
                )
            ...
            for result_data in processed_results_with_soup:
                if result_data:
                    ...
                    if not result_data.get("error"):
                        processed_count += 1
                        await self.emitter.emit(
                            f"📄 Scraped page {processed_count}/{len(tasks)}: {result_data.get('url', 'N/A')}",
                            step_number=7,
                        )
                ...
                if (
                    len([r for r in processed_results if not r.get("error")])
                    >= self.valves.RETURNED_SCRAPPED_PAGES_NO
                ):
                    break
            ...
            await self.emitter.emit(
                status="complete",
                description=f"✅ Web search complete - {final_results_count} pages processed and returned.",
                done=True,
                step_number=8,
            )
    ...
    async def _get_website_content_cached(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if kind is not None and kind.mime == "application/pdf":
                ...
                page_title = (
                    page_title_tag.string if page_title_tag else "No title found"
                )
                page_title = unicodedata.normalize("NFKC", page_title.strip())
                page_title = HelperFunctions.remove_emojis(page_title)
                content_site = HelperFunctions.format_text_with_trafilatura(
                    html_content, self.valves.TRAFILATURA_TIMEOUT
                )
                truncated_content = HelperFunctions.truncate_to_n_words(
                    content_site, self.valves.PAGE_CONTENT_WORDS_LIMIT
                )
    ...
    async def get_website(self, url: str) -> str:
        ...
        if cached_result and CacheValidator.is_valid(
            cached_result, self.valves.CACHE_MAX_AGE_MINUTES
        ):
            ...
            if cached_result:
                logger.info(
                    f"Cache entry for {url} is stale (age > {self.valves.CACHE_MAX_AGE_MINUTES} minutes). Refreshing."
                )
            await self.emitter.emit(
                "🌐 Fetching web content (not cached)", step_number=2
            )
            try:
        ...
        if error:
            await self.emitter.emit(
                status="error",
                description=f"❌ Error accessing page: {error}",
                done=True,
            )
            ...
            if result_data.get(
                "error"
            ):  # Ensure error is bolded if it exists at this stage
...
async def search_web(
    ...
    return await _get_http_tools().search_web(
        query, engines, category, safesearch, time_range
    )
...
@_fastmcp.tool(description="Get the current date and time in the configured timezone.")
...
async def main():
    ...
    while True:
        ...
        try:
            ...
            if method == "initialize":
                            ...
                            "search_web": {
                                ...
                                "inputSchema": {
                                    ...
                                    "properties": {
                                        ...
                                        "engines": {
                                            "type": "string",
                                            "description": "Optional comma-separated list of SearXNG engines (e.g., 'google,wikipedia'). Defaults to instance settings.",
                                        },
                                        "category": {
                                            "type": "string",
                                            "description": "Optional SearXNG category (e.g., 'general', 'images', 'videos', 'files', 'map', 'social media', 'news', 'it', 'science'). Defaults to 'general'.",
                                        },
                                        "safesearch": {
                                            "type": "string",
                                            "enum": ["0", "1", "2"],
                                            "description": "Optional safe search level (0: None, 1: Moderate, 2: Strict). Defaults to instance settings.",
                                        },
                                        "time_range": {
                                            "type": "string",
                                            "enum": ["day", "month", "year"],
                                            "description": "Optional time range filter ('day', 'month', 'year'). Defaults to instance settings.",
                                        },
                                    },
                                    "required": ["query"],
                                },
                ...
                try:  # Tool aliases as per Cline Rule document
                    ...
                    if tool_name == "lookup":
                        ...
                        logger.info(
                            f"Context-sensitive alias: Mapping 'lookup' to '{canonical_name}' based on arguments"
                        )
                        tool_name = canonical_name
                    elif tool_name in alias_map:
                        canonical_name = alias_map[tool_name]
                        logger.info(
                            f"Aliasing tool call from '{tool_name}' to '{canonical_name}'"
                        )
                        tool_name = canonical_name
                    ...
                    if tool_name == "search_web":
                        ...
                        result_text = await tools_instance.search_web(
                            query, engines, category, safesearch, time_range
                        )
                        response["result"] = {
                        ...
                        response["result"] = {
                            "content": [{"type": "text", "text": datetime_str}],
                            "isError": False,
                        }
                    else:
            ...
            logger.exception(
                f"Internal error processing request (ID: {req_id}): {line!r}"
            )
            error_response = {
                "jsonrpc": "2.0",
