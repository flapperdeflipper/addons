...
async def test_hello_roundtrip(client):
    r = await client.post("/api/hello", json={"name": "clanker-a", "note": "hi"}, headers=_auth(client))
...
async def test_thread_flow(client):
    r = await client.post("/api/threads", json={
        "title": "who merges?", "body": "I can", "author": "clanker-a",
        "kind": "proposal", "audience": "all"}, headers=_auth(client))
    tid = r.json()["id"]
    r = await client.post(f"/api/threads/{tid}/messages",
                          json={"author": "clanker-b", "body": "go ahead"}, headers=_auth(client))
    assert r.status_code == 200
    r = await client.post(f"/api/threads/{tid}/close", json={"outcome": "b merges"}, headers=_auth(client))
    assert r.status_code == 200
    r = await client.post(f"/api/threads/{tid}/messages",
                          json={"author": "clanker-a", "body": "late"}, headers=_auth(client))
    assert r.status_code == 400
...
async def test_todos_flow(client):
    r = await client.post("/api/todos", json={"body": "task", "author": "clanker-a"}, headers=_auth(client))
    todo_id = r.json()["id"]
    r = await client.get("/api/todos", params={"name": "clanker-a"}, headers=_auth(client))
...
async def test_claims_flow(client):
    ...
    r = await client.post("/api/claims", json={
        "agent": "clanker-a", "paths": ["/repo/x"], "note": "bumping"}, headers=_auth(client))
    assert r.status_code == 200
    r = await client.get("/api/claims", params={"path": "/repo/x/y"}, headers=_auth(client))
    assert [c["agent"] for c in r.json()] == ["clanker-a"]
    r = await client.request("DELETE", "/api/claims",
                             json={"agent": "clanker-a", "paths": ["/repo/x"]}, headers=_auth(client))
    assert r.status_code == 200
    r = await client.get("/api/claims", params={"path": "/repo/x/y"}, headers=_auth(client))
    assert r.json() == []
...
async def test_check_endpoint(client):
    await client.post("/api/hello", json={"name": "clanker-a"}, headers=_auth(client))
    r = await client.get("/api/check", params={"name": "clanker-b", "since": 0}, headers=_auth(client))
    assert r.status_code == 200
    body = r.json()
...
async def test_bad_kind_maps_to_400(client):
    r = await client.post("/api/threads", json={
        "title": "t", "body": "b", "author": "a", "kind": "rant"}, headers=_auth(client))
    assert r.status_code == 400
